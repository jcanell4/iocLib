...
class DW2HtmlRow extends DW2HtmlInstruction {
    ...
    public function open() {
        ...
        // TODO: Eliminar tot això! copiada la classe box només per fer la prova, cal fer el parser d'una unica fila
        $token = $this->currentToken;


        // TODO

        // va tot embolcallat en <tr>
        //  revisar com s'afegeix el id del wioccl a les files

        // Dividir la fila en cel·les
            // el separador pot ser ^ o |
...
//                return $this->getValueTable($token, $id, $type);
                return $this->getValueTable($token);
...
//    protected function getValueTable($token, $id, $type) {
    protected function getValueTable($token) {
...
//        $value = $this->parseTable($content);
        $value = $this->parseTable($token['raw']);
...
//            }
            ...
            for ($colIndex = 0; $colIndex < count($cols); $colIndex++) {
                ...
                // No cal ajustar pels claudators dels ref perquè el nombre d'espais s'ha de respectar en tots els casos
                if ($start === "  " && $end === "  ") {
                    $cell['align'] = "center";
                } else if ($start === "  ") {
                    $cell['align'] = "right";
